...
class ManualQuestion(db.Model):
  ...
  def __repr__(self):
    return f"ManualQuestion('{self.id}')'"

class UserManualQuestion(db.Model):
  ...
  def __repr__(self):
    return f"UserManualQuestion('{self.id}')'"
...
class VotingQuestion(db.Model):
  ...
  def __repr__(self):
    return f"VotingQuestion('{self.id}')'"

class VotingOption(db.Model):
  __tablename__ = 'VotingOption'
  id = db.Column(db.Integer, primary_key=True)
  option = db.Column(db.String(100), nullable=False)
  date_created = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
...
class UserVotingQuestion(db.Model):
